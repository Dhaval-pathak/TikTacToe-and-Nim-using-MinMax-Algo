...
void printBoardState(int l_board[]){
    for(int i = 0; i < 9; i++){
        printf("%c ", boardToChar(i));
        if (i % 3 == 2) printf("\n");
    }
    printf("\n");
}
...
int minimax(int l_board[], int depth, int maximising){
    ...
    printBoardState(l_board);

    if (gs == X){
        printf("Node Evaluated: X won, Score: 10\n");
        return 10;
    } else if (gs == O){
        printf("Node Evaluated: O won, Score: -10\n");
        return -10;
    } else if (gs == 0){
        printf("Node Evaluated: Draw, Score: 0\n");
        return 0;
    }
    if (depth == 0){
        printf("Node Evaluated: Depth Limit, Score: 0\n");
        return 0;
    }
    ...
    if (maximising){
        ...
        for (int i = 0; i < 9; i++){
            if (legalMovesArr[i]){
                ...
                int tempVal = minimax(tempBoard, depth-1, 0);
                printf("Maximising, Evaluated Node at %d, Value: %d\n", i+1, tempVal);
                val = max(tempVal, val);
            }
        }
        return val;
        ...
        for (int i = 0; i < 9; i++){
            if (legalMovesArr[i]){
                ...
                int tempVal = minimax(tempBoard, depth-1, 1);
                printf("Minimising, Evaluated Node at %d, Value: %d\n", i+1, tempVal);
                val = min(tempVal, val);
            }
        }
        return val;
